...
if (showProcess) {
    canvasFloat = floaty.rawWindow(
        ...
        <vertical w="*">
        <text id="syncTitle" text="移动文件进度" textColor="black" textSize="16sp" singleLine="true"/>
        <progressbar w="*" id="progress" progress="0" style="@style/Base.Widget.AppCompat.ProgressBar.Horizontal" />
        <horizontal w="*" h="100">
        <text id="progressPath" textColor="black" textSize="16sp" margin="8" text="正在获取待移动目录,请稍候......" singleLine="false"/>
        </horizontal>
        <button id="boardClose"  h="auto" w="*" layout_gravity="bottom" text="停止移动" />
        </vertical>
        </frame>
);
    ...
    canvasFloat.boardClose.on("click", () => {
        dialogs.confirm("您确定要停止移动文件吗？").then(value => {
            if (value) {
                canvasFloat.close();
                exit();
            }
...
// 单个移动方法
let singleMove = (soucePath, targetPath) => {
    let resultObj = {};
    // 基础路径
    ...
    // 调用一次
    handlerFilePath(basePath, targetPath);
...
// 批量移动方法
let batchMove = (sourcePathArr, targetPath) => {
    ...
    sourcePathArr.forEach((sourcePath) => {
        let result = singleMove(sourcePath, targetPath);
    ...
    if (showProcess) {
        ui.run(() => {
            canvasFloat.progressPath.attr("text", "待移动目录" + allTargetObj.fileDirArr.length + "个,待移动文件" + allTargetObj.filePathArr.length + "个,请稍候......");
        });
        sleep(500);
    };
        ...
        threads.start(() => {
            setInterval(() => {
                ...
                ui.run(() => {
                    ...
                    canvasFloat.syncTitle.attr("text", "移动文件进度【耗时:" + msToTime(duration) + "】" + "【完成:" + (Number(completeCount / totalCount) * 100).toFixed(2) + "%】");
                    canvasFloat.progressPath.attr("text", "移动到:"+curCopyPath);
                })
                if(copyCompleted){
                    // 调用修改完成同步状态接口
    ...
    allTargetObj.filePathArr.forEach((filePath, index) => {
        ...
        completeCount++;
        files.move(allSourceObj.filePathArr[index], filePath);
    });
    ui.run(() => {
        canvasFloat.progressPath.attr("text", "正在移除源目录,请稍候...");
    });
    allSourceObj.fileDirArr.forEach((filePath, index) => {
        files.removeDir(filePath);
    });
    copyCompleted = true;
}
batchMove(params.sourcePathArr || [], params.targetPath || '');
