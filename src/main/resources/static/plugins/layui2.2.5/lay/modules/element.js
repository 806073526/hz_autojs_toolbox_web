...
layui.define('jquery', function(exports){
  ...
  Element.prototype.tabAdd = function(filter, options){
    ...
    ,titElem = tabElem.find(TITLE)
    ,barElem = titElem.find('.layui-tab-bar')
    ,contElem = tabElem.find('.layui-tab-content')
  ...
  ,call = {
    ...
    tabClick: function(e, index, liElem, options){
      ...
      ,index = index || othis.parent().find('li').index(othis)
      ,parents = options.headerElem ? othis.parent() : othis.parents('.layui-tab').eq(0)
      ,item = options.bodyElem ? $(options.bodyElem) : parents.find('.layui-tab-content').find('.layui-tab-item')
      ,elemA = othis.find('a')
    ...
    ,tabDelete: function(e, othis){
      ...
      ,item = parents.find('.layui-tab-content').find('.layui-tab-item')
      ,filter = parents.attr('lay-filter');
    ...
    ,tabAuto: function(){
      ...
      $('.layui-tab').each(function(){
        ...
        ,item = othis.find('.layui-tab-content').find('.layui-tab-item')
    ...
    ,collapse: function(){
      ...
      if(typeof parents.attr('lay-accordion') === 'string'){
        var show = parents.find('.layui-colla-item').find('.'+SHOW);
        show.siblings('.layui-colla-title').find('.layui-colla-icon').html('&#xe602;');
        ...
        $(NAV_ELEM + elemFilter).each(function(index){
          ...
          itemElem.each(function(){
            ...
            if(child[0] && !oitem.find('.'+NAV_MORE)[0]){
              var one = oitem.find('a');
              one.append('<span class="'+ NAV_MORE +'"></span>');
            }
            
            oitem.off('click', call.clickThis).on('click', call.clickThis); //点击选中
            oitem.find('a').off('click', call.showChild).on('click', call.showChild); //展开二级菜单
            child.find('dd').off('click', call.clickChild).on('click', call.clickChild); //点击子菜单选中
          });
        });
      }
